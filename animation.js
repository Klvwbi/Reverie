...
async function waitForUserClick(seed, canvas) {
  return new Promise((resolve) => {
    function handler(e) {
      ...
      // Exact pixel checks can be frustrating on touch screens, especially
      // when the page is scaled down. Keep the original check and add a
      // generous distance-based hit area around the heart.
      const heartPoint = seed.heart.point;
      const hitRadius = Math.max(45, 30 * seed.heart.scale);
      const dx = point.x - heartPoint.x;
      const dy = point.y - heartPoint.y;
      const inHitArea = dx * dx + dy * dy <= hitRadius * hitRadius;

      let hit = inHitArea;
      if (!hit) {
        try {
          hit = seed.hover(point.x, point.y);
        } catch (_) {
          // Some browsers can reject edge pixel reads; the distance hit area
          // above is enough for the interaction to remain usable.
        }
      }
